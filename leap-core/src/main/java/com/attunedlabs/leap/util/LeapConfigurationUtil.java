...
public class LeapConfigurationUtil {
	...
	final static Logger logger = LoggerFactory
			.getLogger(LeapConfigurationUtil.class);
	...
	public void getPermastoreConfiguration(String configName, Exchange exchange)
		...
		LeapHeader leapHeader = (LeapHeader) exchange.getIn().getHeader(
				LeapHeaderConstant.LEAP_HEADER_KEY);
		try {
			if (leapHeader != null
				...
				RequestContext requestContext = new RequestContext(tenant,
						site, featureGroup, featureName, implName, vendor,
						version);
				IPermaStoreConfigurationService permaConfigService = new PermaStoreConfigurationService();
				Object obj;

				obj = permaConfigService.getPermaStoreCachedObject(
						requestContext, configName);
				Map<String, Object> permaCacheObjectInMap = leapHeader
						.getPermadata();
				permaCacheObjectInMap.put(configName, obj);
			} else {
				throw new PermaStoreConfigRequestException(
						"unable to get the permastore configuration from the configName "
								+ configName
								+ "for the service"
								+ exchange.getIn().getHeader(
										LeapHeaderConstant.SERVICETYPE_KEY)
								+ ", feature : "
								+ exchange.getIn().getHeader(
										LeapHeaderConstant.FEATURE_KEY)
								+ " and feature group : "
								+ exchange.getIn().getHeader(
										LeapHeaderConstant.FEATURE_GROUP_KEY));
			}
		} catch (PermaStoreConfigRequestException e) {
			LeapConfigurationUtil
					.setResponseCode(503, exchange, e.getMessage());
			throw new PermaStoreConfigRequestException(
					"unable to get the permastore configuration from the service "
							+ exchange.getIn().getHeader(
									LeapHeaderConstant.SERVICETYPE_KEY)
							+ ", feature : "
							+ exchange.getIn().getHeader(
	...
	public void getPolicyConfiguration(String configName, Exchange exchange)
		...
		try {
			if (leapHeader != null
				...
				String implName = leapHeader.getImplementationName();
				logger.debug("vendor : " + vendor + ", version : " + version);
				Map<String, Object> policyReqVariableInMap = leapHeader
						.getPolicydata();

				PolicyRequestContext requestContext = new PolicyRequestContext(
						tenant, site, featureGroup, featureName, implName,
						vendor, version);
				...
				for (PolicyEvaluationConfigurationUnit policyEvalConfigUnit : policyEvalConfigUnitList) {
					List<String> reqVarList = policyEvalConfigUnit
							.getReqVarList();
					for (String reqVar : reqVarList) {
						Object reqValue = getRequestVariableValueFromExchange(
								exchange, reqVar);
	...
	public EventRequestContext initializeEventRequestContext(Exchange exchange) {
		...
		eventRequestContext.setHazelcastTransactionalContext(leapHeader
				.getHazelcastTransactionalContext());
	 ...
	 * @return
	 */
	public boolean compareDataContext(RequestContext requestContext,
			DataContext featureDataContext,
		...
		List<RefDataContexts> refDataContextsList = refFeatureDataContext
				.getRefDataContexts();
		for (RefDataContexts refDataContexts : refDataContextsList) {
			...
			if (featureGroup.equalsIgnoreCase(requestContext.getFeatureGroup())
					&& featureName.equalsIgnoreCase(requestContext
							.getFeatureName())) {
				List<RefDataContext> refDataContextList = refDataContexts
						.getRefDataContext();
				for (RefDataContext refDataContext : refDataContextList) {
					if (refDataContext.getDbBeanRefName().equalsIgnoreCase(
							dbBeanRefName)
							&& refDataContext.getDbType().equalsIgnoreCase(
									dbType)
							&& refDataContext.getDbHost().equalsIgnoreCase(
									dbHost)
							&& refDataContext.getDbPort().equalsIgnoreCase(
									dbPort)
							&& refDataContext.getDbSchema().equalsIgnoreCase(
									dbSchema)) {
						flag = true;
					} else {
						flag = false;
	...
	public void getIntegrationPipeContext(Exchange exchange) {
		...
		String exchangeBody = (String) body
				.get(LeapGenericConstant.PIPE_CONTEXT_KEY);
	...
	public void setOldleapHeader(Exchange exchange) {
		LeapHeader leapHeader = (LeapHeader) exchange.getIn().getHeader(
				LeapHeaderConstant.LEAP_HEADER_KEY);
		logger.debug("setting the leap header in exchange : "
				+ leapHeader.toString());
		logger.debug("list of  header in exchange : "
	 ...
	 */
	public void getIntegrationPipelineConfiguration(String configName,
			RequestContext reqcontext, LeapHeader leapHeader, Exchange exchange)
			throws IntegrationPipelineConfigException {
		...
		RequestContext request = new RequestContext(reqcontext.getTenantId(),
				reqcontext.getSiteId(), reqcontext.getFeatureGroup(),
				reqcontext.getFeatureName(),
				reqcontext.getImplementationName(), reqcontext.getVendor(),
				reqcontext.getVersion());
		if (pipelineConfigUnit == null) {
			...
			pipelineConfigUnit = pipeLineConfigurationService
					.getIntegrationPipeConfiguration(request, configName);
			logger.debug("pipeLineConfiguration Unit After setting the tenant And Site as All : "
					+ pipelineConfigUnit.toString());
		}

		logger.debug(".inIPipelineLeapConfigUtil.." + pipelineConfigUnit
				+ "ConfigurationName in leapConfigUtil.." + configName);
		// ......TestSnipet....
		...
		Map<String, Object> integrationCahedObject = leapHeader
				.getIntegrationpipelineData();
		if (integrationCahedObject == null)
			integrationCahedObject = new HashMap<String, Object>();
		integrationCahedObject.put(LeapHeaderConstant.PIPELINE_CONFIG_KEY,
				pipelineConfigUnit);
		logger.debug(".inLeapUtil cachedObject..Check before putting"
				+ integrationCahedObject);
	}// ..end of the method

	private Object getRequestVariableValueFromExchange(Exchange exchange,
			String reqVar) {
		...
		if (reqvalue == null) {
			logger.debug(reqVar
					+ " not found in exchange so checking in leapHeader data ");
			Map<String, Object> geniricdata = leapHeader.getGenricdata();
			try {
				JSONArray jsonArray = (JSONArray) geniricdata
						.get(LeapHeaderConstant.DATA_KEY);
				int jsonLen = jsonArray.length();
				logger.debug("data's in json array for key data is : "
						+ jsonLen);
				for (int i = 0; i < jsonLen; i++) {
					...
					if (reqvalue == null) {
						logger.debug("checking in  leapHeader using " + "$"
								+ reqVar);
						reqvalue = (Object) jobj.get("$" + reqVar);
					}
				}
		...
		if (reqvalue == null) {
			logger.debug(reqVar
					+ " not found in exchange so checking in exchange property data ");
	 ...
	 */
	public static Properties loadingPropertiesFile()
			throws PropertiesConfigException {
	...
	public static void setResponseCode(int key, Exchange exchange,
		...
		jsonObj.put("http response code ",
