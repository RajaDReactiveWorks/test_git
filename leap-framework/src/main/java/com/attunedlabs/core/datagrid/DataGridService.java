...
import static com.attunedlabs.eventframework.abstractbean.util.CassandraUtil.PAAS_CASSANDRA_DEPLOYMENT_ENVIRONMENT_KEY;
			...
			synchronized (DataGridService.class) {
				...
				try {
					...
					if(deployemntEnv!=null && !(deployemntEnv.isEmpty()) && deployemntEnv.length()>0 && deployemntEnv.equalsIgnoreCase(PAAS_CASSANDRA_DEPLOYMENT_ENVIRONMENT_KEY)){
						...
						hcInstance.getPartitionService().addPartitionLostListener( new ConsoleLoggingPartitionLostListener() );
