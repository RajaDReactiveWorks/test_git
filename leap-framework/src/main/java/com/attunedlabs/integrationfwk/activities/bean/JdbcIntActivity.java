...
public class JdbcIntActivity extends AbstractMetaModelBean {
	...
	protected void processBean(Exchange exchange) throws JdbcIntActivityQueryProcessingException {
		...
		try {
			 ...
			 * 
			 */
			setDataSource(JdbcIntActivityMetaModelUtil.getDataSource(exchange.getContext(), getDatacontext(exchange).getDbBeanRefName()));
			...
			if ("SELECT".equals(operation)) {
				...
				else if(newQuery.toLowerCase().contains("*"))
				{
					...
					colums = (ArrayList<String>) datacontext.getDefaultSchema().getTableByName(tableName).getColumnNames();
					String xmlEnriched = configHelper.processxmlEnrichment(xmlDocument, configObject, colums, listOfValues);
					exchange.getIn().setBody(xmlEnriched);
				}
			} else {
	...
	private static DataContext getDatacontext(Exchange exchange) throws Exception {
		LeapMetaModelBean bean = new LeapMetaModelBean();
		return bean.loadDataContext(exchange);
	}
