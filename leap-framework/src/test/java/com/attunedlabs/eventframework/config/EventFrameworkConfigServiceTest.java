...
public class EventFrameworkConfigServiceTest {
	...
	public void testAddConfigurationEventWithServiceProducer() throws EventFrameworkConfigParserException,
		 ...
		 * Assert.
		 * assertEquals("Event{PRINTSERVICE} should be available in ProducerConfigService Map with "
	...
	public void testAddAndGetEventSubscriptionConfiguration() throws EventFrameworkConfigParserException,
			EventFrameworkConfigurationException, ConfigPersistenceException {
		ConfigurationContext configContext = getConfigurationContext(6);
		List<SubscribeEvent> eventSubscriptionList = eventFrameworkConfig.getEventSubscription().getSubscribeEvent();
		SubscribeEvent printSerEventSubs = null;

		for (SubscribeEvent eventSubs : eventSubscriptionList) {
			if (eventSubs.getSubscriptionId().equalsIgnoreCase("TestSubscriber")) {
		...
		SubscribeEvent eventsubs = eventConfigService.getEventSubscriptionConfiguration(configContext,
				printSerEventSubs.getSubscriptionId());
	...
	public void testChangeStatusOfConfigurationEventSubscription() throws EventFrameworkConfigParserException,
		...
		Assert.assertNotNull(
				"EventSubscription is not null from event service because consfig is enabled Subscription : "
						+ eventsubs);

		boolean isEnable = eventConfigService.changeStatusOfEventSubscriptionConfiguration(configContext,
				eventsubs.getSubscriptionId(), false);

		eventsubs = eventConfigService.getEventSubscriptionConfiguration(configContext,
				printSerEventSubs.getSubscriptionId());

		Assert.assertTrue("event Subscriber Configuration should be enabled =False ", isEnable);
		Assert.assertNull("eventConfiguration is removed form hazelcast and config node status has changed ",
				eventsubs);

		isEnable = eventConfigService.changeStatusOfEventSubscriptionConfiguration(configContext,
				nextTest.getSubscriptionId(), true);

		nextTest = eventConfigService.getEventSubscriptionConfiguration(configContext, nextTest.getSubscriptionId());

		Assert.assertTrue("event Subscriber Configuration should be enabled =True ", isEnable);
		Assert.assertNotNull("eventConfiguration is added form hazelcast and config node status has changed ",
				eventsubs);
	...
	public void testApplicableNodeId() throws InvalidNodeTreeException, ConfigPersistenceException {
		...
		/*
		 * Assert.assertEquals("ApplicableNodeId at Feature should be",
		 * GenericTestConstant.getFeatureConfigNodeId(),
	...
	public void TestDeleteEventSubscriptionConfigurationIfEnabled()
		...
		boolean isDeleted = eventConfigService.deleteEventSubscriptionConfiguration(configContext,
				printSerEventSubs.getSubscriptionId());
		Assert.assertTrue("event configuration deleted from hazelcase and frm confignode data permanantly", isDeleted);
	...
	public void TestDeleteDispactherChanelConfigurationIfDisabled()
		...
		LeapConfigurationServer leapConfigurationServer;
		...
		try {
			...
			leapConfigurationServer.deleteConfiguration(configContext.getTenantId(), EventFrameworkConfigurationUnit
