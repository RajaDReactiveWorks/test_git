...
public class DatabaseMetaDataWrapper
{
    ...
    {
    	_log.debug("LiteralString: "+literalString);
        String escape = getMetaData().getSearchStringEscape();
        _log.debug("EscapeString: "+escape);
        if (escape == "")
        {
...
_log.debug("eachcharacter in else-escape: "+c);
                switch (c)
                {
                    case '\\':
                        quotedEscape.append("\\\\");
...
_log.debug("QuotedEscapeString: "+quotedEscape.toString());
    ...
    public static void main(String[] a){
    	DatabaseMetaDataWrapper dataWrapper = new DatabaseMetaDataWrapper();
    	System.out.println(dataWrapper.searchStringPattern.matcher("SYS_NT0q/QcEraQaatSL7h+M1a2A==").replaceAll("/$0"));
    }
