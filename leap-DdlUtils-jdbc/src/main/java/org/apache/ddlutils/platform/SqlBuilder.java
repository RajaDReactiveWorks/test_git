...
public abstract class SqlBuilder {
	...
	protected final static Log _log = LogFactory.getLog(SqlBuilder.class);
	...
	/**
	 * The Database that this builder belongs to. Added by AttunedLabs to support
	 * partitioning
	 */
	private String _database;
	/**
	 * The current Writer used to output the SQL tableSpace to. Added by AttunedLabs
	 * to support partitioning
	 */
	private static Writer _tableSpaceWriter;
	...
	/**
	 * Returns the default value database.
	 * 
	...
	/**
	 * Returns the tableSpaceWriter that the DDL is printed to.
	 * 
	 * @return the _tableSpaceWriter
	 */
	public Writer getTableSpaceWriter() {
		return _tableSpaceWriter;
	}

	/**
	 * Sets the tableSpaceWriter for printing the DDL to.
	 * 
	 * @param _tableSpaceWriter
	 *            the _tableSpaceWriter to set
	 */
	public void setTableSpaceWriter(Writer tableSpaceWriter) {
		this._tableSpaceWriter = tableSpaceWriter;
	}

	/**
	 * Sets the database name to the builder.
	 * 
	 * @param _database
	 *            the _database to set
	 */
	...
	/**
	 * Returns the maximum number of characters that a column name can have. This
	 * method is intended to give platform specific builder implementations more
	 * control over the maximum length.
	 * 
	 * @return The number of characters, or -1 if not limited
	...
	/**
	 * Outputs the DDL required to drop and (re)create all tables in the database
	 * model.
	 * 
	 * @param database
	 *            The database model
	...
	public void createTables(Database database, CreationParameters params, boolean dropTables) throws IOException {
		...
		// we're writing the external foreignkeys last to ensure that all referenced
		// tables are already defined
	 ...
	 * Database-specific implementations might redefine this method though it
	 * usually suffices to redefine the {@link #writeCastExpression(Column, Column)}
	 * method.
	 * 
	 * @param sourceTable
	 *            The source table
	...
	/**
	 * Writes the indexes for the given table using external index creation
	 * statements.
	 * 
	 * @param table
	 *            The table
	...
	public void dropTables(Database database) throws IOException {
		...
		// * determine an order in which the tables can be dropped safely (via the
		// foreignkeys)
	 ...
	 *            Contains the column values to identify the row to update
	 * @param newColumnValues
	 *            Contains the values for the columns to update
	 * @param genPlaceholders
	 *            Whether to generate value placeholders for a prepared statement
	 *            (both for the pk values and the object values)
	 * @return The update sql
	 ...
	 * completely empty, then the SQL will not have a WHERE clause. The SQL will
	 * contain the columns in the order defined in the table.
	 * 
	 * @param table
	 *            The table
	 * @param pkValues
	 *            The primary key columns to use, and optionally their values
	 * @param genPlaceholders
	 *            Whether to generate value placeholders for a prepared statement
	...
	protected void writeTableCreationStmt(Database database, Table table, Map parameters) throws IOException {
		...
		_log.debug("database : " + database);

		if (_database.equalsIgnoreCase("MsSql") && table.getPartitionSchema() != null
				&& table.getPartitionSchema().getName() != null) {
			AttunedLabsSqlBuilder.writeMsSqlCreatePartition(table, database);
		}
		...
		if (table.isPartitionSupported()) {
			AttunedLabsSqlBuilder.writeEmbeddedPartitionKeysStmt(table, _database);
		}

	}

	public String writeEmbeddedTableSpaceStmt(Table table, Database database) throws IOException {
		AttunedLabsSqlBuilder.tableSpacesQueryBuilder(table, database);
		_log.debug("_tableSpaceWriter.toString() : " + _tableSpaceWriter.toString());
		return _tableSpaceWriter.toString();
	}
	 ...
	 *            The column
	 * @param nativeType
	 *            Overrides the native type of the column; can include the size
	 *            placeholder
	 * @return The full SQL type string including the size
	 */
	protected String getSqlType(Column column, String nativeType) {
		...
		if (_database.equalsIgnoreCase("POSTGRESSQL")) {
			if(nativeType.equalsIgnoreCase("VARCHAR"))
				if(sizeSpec.equalsIgnoreCase("MAX"))
					return "VARCHAR(MAX)";
		}
		if (_database.equalsIgnoreCase("MYSQL")) {
			if(nativeType.equalsIgnoreCase("VARCHAR"))
				if(Integer.parseInt(sizeSpec) > 21000)
					return "MEDIUMTEXT";
			if(nativeType.equalsIgnoreCase("NVARCHAR"))
				if(Integer.parseInt(sizeSpec) > 19000)
					return "MEDIUMTEXT";
		}
		if (_database.equalsIgnoreCase("MSSQL")) {
			if(nativeType.equalsIgnoreCase("VARCHAR"))
				if(Integer.parseInt(sizeSpec) > 8000)
					return "NVARCHAR(MAX)";
			if(nativeType.equalsIgnoreCase("NVARCHAR"))
				if(Integer.parseInt(sizeSpec) > 4000)
					return "NVARCHAR(MAX)";
		}
		if (_database.equalsIgnoreCase("ORACLE")) {
			if(nativeType.equalsIgnoreCase("VARCHAR2") || nativeType.equalsIgnoreCase("VARCHAR"))
				if(Integer.parseInt(sizeSpec) > 4000)
					return "NCLOB";
			if(nativeType.equalsIgnoreCase("NVARCHAR2"))
				if(Integer.parseInt(sizeSpec) > 2000)
					return "NCLOB";
		}
	...
	/**
	 * Returns the size specification for the given column. If the column is of a
	 * type that has size or precision and scale, and no size is defined for the
	 * column itself, then the default size or precision/scale for that type and
	 * platform is used instead.
	 * 
	 * @param column
	 *            The column
	...
	protected String getSizeSpec(Column column) {
		...
		if (sizeSpec instanceof String)
			if (((String) sizeSpec).equalsIgnoreCase("MAX")) {
				return "MAX";
			}
		if (sizeSpec == null) {
	...
	protected void writeColumnDefaultValueStmt(Table table, Column column) throws IOException {
		...
		if (parsedDefault != null) {
			...
			print(" DEFAULT ");
			writeColumnDefaultValue(table, column);
//		} else if(_database.equalsIgnoreCase("MYSQL") && column.getType().equalsIgnoreCase("TIMESTAMP")) {
//		} else if(parsedDefault == null && column.getType().equalsIgnoreCase("TIMESTAMP")) {
//			print(" DEFAULT 0");
		}
	}
	 ...
	 * attributes checked. Currently default values are compared, and null and empty
	 ...
	 * Prints a newline.
	 */
	protected static void println() throws IOException {
	 ...
	 *            The text to print
	 */
	protected static void print(String text) throws IOException {
		_writer.write(text);
	}
	 ...
	 *            The text to print
	 */
	protected static void printTS(String text) throws IOException {
		_tableSpaceWriter.write(text);
	}

	/**
	 * Prints a newline.
	 */
	protected static void printTSln() throws IOException {
		printTS(LINE_SEPARATOR);
	}
	 ...
	 *            The text to print
	 */
	protected static void printTSln(String text) throws IOException {
		printTS(text);
		printTSln();
	}
	 ...
	 *            The text to print
	 */
	protected static void println(String text) throws IOException {
		print(text);
		println();
	}
