...
public class CassandraUtils {
	...
	public static Row toRow(com.datastax.driver.core.Row dbObject, DataSetHeader header,
			Map<String, TypeToken<?>> typeConverters) {
		if (dbObject == null) {
			return null;
		}
		...
		for (int i = 0; i < values.length; i++) {
			...
			values[i] = getColumnValue(key, dbObject, typeConverters);
		}
	...
	private static Object getColumnValue(String columnName, com.datastax.driver.core.Row row,
		...
		case VARINT:
			return row.getVarint(columnName);
		case LIST:
			if (typeConverters != null && typeConverters.containsKey(columnName))
				return row.get(columnName, typeConverters.get(columnName));
			else
