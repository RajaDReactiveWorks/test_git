...
public class CassandraDataContextFactory extends AbstractDataContextFactory {
	...
	public DataContext create(DataContextProperties properties, ResourceFactoryRegistry resourceFactoryRegistry)
		...
		final Map<String, TypeToken<?>> typeTokenConverters = (Map<String, TypeToken<?>>) map.get("type-token");

		CassandraCustomDataContext cassandraCustomDataContext = null;

		if (!map.containsKey(DataContextPropertiesImpl.PROPERTY_TABLE_DEFS))
			cassandraCustomDataContext = new CassandraCustomDataContext(cluster, keySpace);
		else
			cassandraCustomDataContext = new CassandraCustomDataContext(cluster, keySpace, properties.getTableDefs());

		if (typeTokenConverters != null)
			cassandraCustomDataContext.setTypeTokenConverters(typeTokenConverters);
		return cassandraCustomDataContext;
	}
