...
public class ConfigurationTreeNode implements Serializable 
{
	...
	public ConfigurationTreeNode(Integer nodeId, String nodeName, String type,	int level,String version,int primaryFeatureId) {
		this.nodeId = nodeId;
		this.nodeName = nodeName;
		this.type = type;
		this.level = level;
	...
	public boolean addChildren(ConfigurationTreeNode childNode)
	{
		...
		//If Child Node has this as Parent than add other wise iterate through the List of Child and than Add
		if(childNode.parentNodeId.intValue()==this.nodeId.intValue()){
			childNodes.add(childNode);
			childNode.setParentNode(this);
			//System.out.println("ChildNode-Added ChildNodeId="+childNode.getNodeId()+"---to Parent NodeId="+nodeId);
			return true;
		}else{
	...
	{
		return "Node [ nodeId=" + nodeId + ", nodeName=" + nodeName + ", type="
				+ type + ", level=" + level + ", parentNodeId=" + parentNodeId+ ", version=" + version+ ", primaryFeatureId=" + primaryFeatureId
				+ "{"+childNodes+"]]";
	}
