...
public class EventFrameworkConfigHelper {
	...
	public Event getEventConfiguration(ConfigurationContext configContext, String forEventId)
		...
		try {
			...
			for (int i = searchStartLevel; i > 1; i--) {
				...
				logger.debug("getApplicableNodeId ");
				Integer nodeId = getApplicableNodeId(configContext);
				logger.debug("getEventConfigGroupKey");
	...
	public DispatchChanel getDispatchChanelConfiguration(ConfigurationContext configContext, String dispatchChanelId)
		...
		try {
			...
			for (int i = searchStartLevel; i > 1; i--) {
				...
				logger.debug(".getDispatchChanelConfiguration() searching at level " + i + " ConfigUnit="
						+ evtFwkConfigUnit);
				if (evtFwkConfigUnit != null) {
		...
		try {
			...
			if (!isEnable) {
				...
				EventFrameworkConfigurationUnit evtConfigUnit = new EventFrameworkConfigurationUnit(
						...
						configNodeData.getParentConfigNodeId(), isEnable, dispatchChanel);
	...
	public boolean deleteDipatcherChanelConfiguration(ConfigurationContext configContext, String dispatchChanelId)
		...
		try {
			...
			EventFrameworkConfigurationUnit evtFwkConfigUnit = (EventFrameworkConfigurationUnit) roiConfigurationServer
					.getConfiguration(configContext.getTenantId(), eventGroupKey, dispatchChanelId);
	...
	public void addEventFrameworkConfiguration(ConfigurationContext configContext,
		...
		try {
			...
			ConfigNodeData configNodeData = new ConfigNodeData(configNodeId, eventSubscriptionConfig.getEventId(),
					evtSubscriptionFwkXMLStr, EventFrameworkConstants.EF_EVENTSUBSCRIPTION_CONFIG_TYPE);// ConfigNodeData(Integer
																										// parentConfigNodeId,
																										// String
																										// configName,
																										// String
			...
			IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
			ConfigNodeData loadedConfigNodeData = configPersistenceService.getConfigNodeDatabyNameAndNodeId(
					configNodeId, eventSubscriptionConfig.getEventId(),
			...
			if (loadedConfigNodeData == null) {
				...
				if (event != null) {
					...
					EventFrameworkConfigurationUnit evtConfigUnit = new EventFrameworkConfigurationUnit(
							configContext.getTenantId(), configContext.getSiteId(), configNodeId,
							eventSubscriptionConfig.isEnabled(), eventSubscriptionConfig);
					evtConfigUnit.setDbconfigId(configDataId);
					loadConfigurationInDataGrid(evtConfigUnit);
				} else {
					logger.debug("event subscrition for the event : " + eventSubscriptionConfig.getEventId()
							+ " failed because event doesnot exist ");
					throw new EventFrameworkConfigurationException("event subscrition for the event : "
							+ eventSubscriptionConfig.getEventId() + " failed because event doesnot exist ");
				}
			} else {
				logger.debug("event subscrition for the event : " + eventSubscriptionConfig.getEventId()
						+ " failed because event subscription already exist ");
				throw new EventFrameworkConfigurationException("event subscrition for the event : "
						+ eventSubscriptionConfig.getEventId() + " failed because event sunscription already exist ");
			}
			// }
		} catch (InvalidNodeTreeException | ConfigPersistenceException | EventFrameworkConfigParserException e) {
			throw new EventFrameworkConfigurationException(
					"Failed to add EventSubscriptionConfiguration for Event with eventId"
							+ eventSubscriptionConfig.getEventId(),
					e);
		}
	...
	public boolean changeStatusOfEventSubscriptionConfiguraion(ConfigurationContext configurationContext,
		...
		try {
			...
			if (!isEnable) {
				...
				EventSubscription eventSubscription = eventFramework.getEventSubscriptions().getEventSubscription()
						.get(0);
				...
				boolean updateConfiData = configPersistenceService.updateConfigdataInConfigNodeData(
						updatedEventFrameworkString, applicableNodeId, eventSubscription.getEventId(),
						EventFrameworkConstants.EF_EVENTSUBSCRIPTION_CONFIG_TYPE);
				String eventSubscriptionGroupKey = EventFrameworkConfigurationUnit
	 ...
	 * @throws EventFrameworkConfigParserException
	 */
	private String convertEventFrameworkObjectToString(EventSubscription eventSubscription)
			throws EventFrameworkConfigParserException {
		logger.debug("inside convertEventFrameworkXmlStringToObject of EventFrameworkConfigHelper");
		EventFrameworkXmlHandler eventFrameworkXmlHandler = new EventFrameworkXmlHandler();
