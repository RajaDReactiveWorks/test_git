...
import com.getusroi.featuredeployment.FeatureDeploymentServiceException;
import com.getusroi.featuredeployment.IFeatureDeployment;
...
public abstract class AbstractMetaModelBean extends AbstractROICamelBean {
	...
	/**
	 * This method is used to get com.getusroi.datacontext.jaxb.DataContext
	 * Object
	 * 
	...
	protected com.getusroi.datacontext.jaxb.DataContext getFeatureDataContext(Exchange exchange)
		...
		/**
		 * In some cases the AbstractMetaModel bean getDataContext gets 'null'
		 * as implementation, hence below condition
		 **/
		if (meshheader.getImplementationName() == null) {
			...
			FeatureDeployment featureDeployment = featureDeploymentservice.getActiveAndPrimaryFeatureDeployedFromCache(
					meshheader.getTenant(), meshheader.getSite(), meshheader.getFeatureName());
			newRequestContext = new RequestContext(meshheader.getTenant(), meshheader.getSite(),
			...
			logger.info("newRequestContext :" + newRequestContext);
		} else {
			logger.info("inside else");
			newRequestContext = meshheader.getRequestContext();
			logger.info("newRequestContext :" + newRequestContext);
		}
		com.getusroi.datacontext.jaxb.DataContext datacontext = getDataContextObjectOfFeature(newRequestContext);
		logger.info("datacontext :" + datacontext);
		return datacontext;
	}
	 ...
	 * com.getusroi.datacontext.jaxb.FeatureDataContext Object
	 * 
	 * @param requestContext
	 *            : RequestCOntext Object of a feature
	 * @return com.getusroi.datacontext.jaxb.FeatureDataContext
	...
	protected boolean compareDataContext(RequestContext requestContext,
			...
			com.getusroi.datacontext.jaxb.DataContext featureDataContext, FeatureDataContext refFeatureDataContext) {
	...
	/**
	 * This method is used to get the com.getusroi.datacontext.jaxb.DataContext
	 ...
	 * @throws Exception
	 */
	protected com.getusroi.datacontext.jaxb.DataContext getReferenceFeatureDataContext(String featureGroup,
		...
		ConfigurationTreeNode fgconfigNodeTree = tenantTreeService.getPrimaryVendorForFeature(meshheader.getTenant(),
				meshheader.getSite(), featureGroup, feature);
		String vendorName = fgconfigNodeTree.getNodeName();
		String version = fgconfigNodeTree.getVersion();
		RequestContext requestContext = new RequestContext(meshheader.getTenant(), meshheader.getSite(), featureGroup,
				feature, meshheader.getImplementationName(), vendorName, version);
	...
	protected com.getusroi.datacontext.jaxb.DataContext getReferenceFeatureDataContext(String featureGroup,
		...
		MeshHeader meshheader = (MeshHeader) exchange.getIn().getHeader(MeshHeaderConstant.MESH_HEADER_KEY);
		RequestContext requestContext = new RequestContext(meshheader.getTenant(), meshheader.getSite(), featureGroup,
				feature, meshheader.getImplementationName(), vendor, version);
		com.getusroi.datacontext.jaxb.DataContext dataContext = getDataContextObjectOfFeature(requestContext, dbName);
	...
	/**
	 * This method is used to get the com.getusroi.datacontext.jaxb.DataContext
	 * of reference feature #TODO,till now its not decided how many data context
	 * a feature can have,in xml we have provided a support for multiple but in
	 * code implementation we are taking it as only one.
	 * 
	 * @param requestContext
	...
	private com.getusroi.datacontext.jaxb.DataContext getDataContextObjectOfFeature(RequestContext requestContext)
		...
		DataContextConfigurationUnit dataContextConfigurationUnit = dataContextConfigService
				.getDataContextConfiguration(requestContext);
		logger.info("dataContextConfigurationUnit :" + dataContextConfigurationUnit);
		FeatureDataContext featureDataContext = (FeatureDataContext) dataContextConfigurationUnit.getConfigData();
		List<com.getusroi.datacontext.jaxb.DataContext> dataContextList = featureDataContext.getDataContexts()
				.getDataContext();
		...
		logger.info("completed for " + dataContextforFeature);
		logger.info("dataContextforFeature " + dataContextforFeature);
	 ...
	 * decided how many data context a feature can have,in xml we have provided
	 * a support for multiple but in code implementation we are taking it as
	 * only one.
	 * 
	 * @param requestContext
	 *            : Request Context Object of current feature
