...
public class MeshMetaModelHelper {

	private static final Logger logger = LoggerFactory.getLogger(AbstractROICamelBean.class);

	/**
	 * To restrict a construct of instance
	 */
	private MeshMetaModelHelper() {
	}
	 ...
	 */
	protected static Query buildJoinQueries(JdbcDataContext jdbcDataContext, String queryFromObject, String joinObject,
		...
		switch (joinType) {
		case MeshMMConstants.INNER_JOIN:
	...
	protected static String getMappedStringFromList(String substitutableString, List<?> substitutableValues) {
		...
		String temp = "";
		if (!MeshMetaModelHelper.isEmptyList(substitutableValues)) {
	...
	protected static List<String> getColumsbyTable(Table table) {
		return Arrays.asList(table.getColumnNames());
	}// ..end of the method
