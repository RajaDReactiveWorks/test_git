...
import static com.getusroi.eventframework.abstractbean.util.CassandraUtil.*;
...
public abstract class AbstractCassandraBean extends AbstractROICamelBean{
	...
	protected Connection getCassandraConnection() throws CassandraConnectionException{
		...
		try {
			...
			if(prop!=null){
				...
				}else if(deployemntEnv!=null && !(deployemntEnv.isEmpty()) && deployemntEnv.length()>0 && deployemntEnv.equalsIgnoreCase(LOCAL_CASSANDRA_DEPLOYMENT_ENVIRONMENT_KEY)){
					return getLocalCassandraConnection();
				}else{
					throw new CassandraConnectionException("Unsupportable deployment environment for cassandra "+deployemntEnv+". Please provide either PAAS or Local");
				}
			}else{
			...
			throw new CassandraConnectionException("unable to get the connection object for cassandra : "+prop.getProperty(APPS_DEPLOYEMENT_ENVIRONMENT_CONFIG_PROPERTY_FILE),e2);
		}
		...
		try {
			...
			if(prop!=null){
				...
				if(deployemntEnv!=null && !(deployemntEnv.isEmpty()) && deployemntEnv.length()>0 && deployemntEnv.equalsIgnoreCase(PAAS_CASSANDRA_DEPLOYMENT_ENVIRONMENT_KEY)){
					...
					throw new CassandraClusterException("Unsupportable deployment environment for cassandra "+deployemntEnv);
				}
			}else{
				throw new CassandraClusterException("Error while fetching the cassadra deployemnt detail ");
			}
		} catch (ConnectionConfigurationException e2) {
			throw new CassandraClusterException("unable to get the connection object for cassandra : "+prop.getProperty(APPS_DEPLOYEMENT_ENVIRONMENT_CONFIG_PROPERTY_FILE),e2);
		}
		...
		try {
			...
			try {
				...
				try {
					...
					throw new CassandraConnectionException("URL to get the connection object for cassandra is invalid : "+prop.getProperty(URL_KEY),e);
				}
	...
	}//end of method getLocalCassandraConnection
		...
		try {
			...
			try {
					...
					if(cassandraIP!=null && !(cassandraIP.isEmpty()) && cassandraIP.length()>0){
						...
						if((cassandraPort!=null && !(cassandraPort.isEmpty()) && cassandraPort.length()>0) && (cassandraKeysapce!=null && !(cassandraKeysapce.isEmpty()) && cassandraKeysapce.length()>0)){
						//create url
						 String url=JDBC_CASSANDRA_PROTOCOL+cassandraIP+JDBC_CASSANDRA_PROTOCOL_COLON_SEPERATED+cassandraPort+JDBC_CASSANDRA_PROTOCOL_SEPERATED+cassandraKeysapce;
						...
						try {
							...
							throw new CassandraConnectionException("unable to create cassandra connection object with url : "+url);
						}						
						}else{
							throw new CassandraConnectionException("Unable to create connection for cassandra with port and and keyspace : "+cassandraPort+", "+cassandraKeysapce);
						}
					}else{
						throw new CassandraConnectionException("Unable to create connection for cassandra with an IP given in system evironment : "+cassandraIP);
					}				
			} catch (ClassNotFoundException e) {
				throw new CassandraConnectionException("unable to load the driver name for cassandra : "+prop.getProperty(DRIVER_CLASS_KEY),e);
			}
		} catch (ConnectionConfigurationException e1) {
			throw new CassandraConnectionException("unable to get the connection object for cassandra : "+prop.getProperty(PAAS_CONFIG_PROPERTY_FILE),e1);
		}
	...
	private Cluster getPAASCassandraCluster() throws CassandraClusterException{
		...
		try {
			...
			try{
				...
				throw new CassandraClusterException("unable to connect to the host for cassandra host and port : "+host+", "+port,e);
			}
			}else{
				throw new CassandraClusterException("Unable to create connection for cassandra with an IP given in system evironment : "+host);

			}
		} catch (ConnectionConfigurationException e1) {
			throw new CassandraClusterException("unable to get the connection object for cassandra : "+prop.getProperty(PAAS_CONFIG_PROPERTY_FILE),e1);
		}		
