...
public class PolicyConfigurationService extends GenericApplicableNode implements IPolicyConfigurationService {
	...
	public void addPolicyConfiguration(ConfigurationContext configurationContext, Policy policyConfig)
			throws PolicyConfigurationException {
		logger.debug(".addPolicyConfiguration() tree is " + configurationContext.getTenantId() + "/"
				+ configurationContext.getSiteId() + " for policy=" + policyConfig.getPolicyName());
		...
		try {
			...
			if ((vendorName != null && !(vendorName.isEmpty()) && !(vendorName.equalsIgnoreCase("")))
				...
				configNodeId = getApplicableNodeIdVendorName(tenantId, siteId, featureGrp, featureName, implName,
						vendorName, version);
			} else {
			...
			ConfigNodeData loadedConfigNodeData = configPersistenceService
					.getConfigNodeDatabyNameAndNodeId(configNodeId, policyName, PolicyConstant.POLICY_CONFIG_TYPE);
			...
			if (loadedConfigNodeData == null) {
				...
				throw new PolicyConfigurationException(
						"pOlicy already exist for policyName=" + policyName + "--tree=" + tenantId + "/" + siteId + "/"
								+ featureGrp + "/" + featureName + "/" + configurationContext.getImplementationName());
			}
			...
			PolicyConfigurationUnit psConfigUnit = new PolicyConfigurationUnit(tenantId, siteId, configNodeId, true,
					policyConfig, null);
		...
		} catch (InvalidNodeTreeException | ConfigPersistenceException | PolicyConfigXMLParserException
				| PolicyFactBuilderException e) {
	...
	public PolicyConfigurationUnit getPolicyConfigurationUnit(PolicyRequestContext polRequestContext, String policyName)
			throws PolicyConfigurationException {
		logger.debug(
				".getPolicyConfigurationUnit() PolicyName=" + policyName + "+-RequestContext=" + polRequestContext);
		try {
			...
			ConfigurationUnit configUnit = configServer.getConfiguration(polRequestContext.getTenantId(), policyGrpKey,
					policyName);
	...
	public Object getPolicyResponseData(PolicyRequestContext policyRequestContext, String policyName)
	...
	private ConfigNodeData buildConfigNodeData(Policy policy, Integer configNodeId)
	...
	private void loadConfigurationInDataGrid(PolicyConfigurationUnit polConfigUnit)
			throws PolicyConfigurationException {
		...
		try {
			...
			throw new PolicyConfigurationException("Failed to Upload in DataGrid configName=" + polConfigUnit.getKey(),
					e);
		}
	}
	...
	public boolean changePolicyStatus(ConfigurationContext configurationContext, String policyName, boolean isEnable)
			throws PolicyConfigurationException, PolicyConfigXMLParserException {

		Integer applicableNodeId;
		PolicyRequestContext policyRequestContext = new PolicyRequestContext(configurationContext.getTenantId(),
		...
		try {
			...
			// Disable Request
			if (!isEnable) {
				// We have to Disable policyConfig hence remove from DataGrid
				// and update DB as disabled Configuration
		...
		} catch (ConfigPersistenceException | PermaStoreConfigurationException | ConfigServerInitializationException
				| PolicyFactBuilderException e) {
	...
	private void enableAndLoadPolicyConfig(PolicyRequestContext reqCtx, ConfigNodeData configNodeData)
			throws ConfigPersistenceException, PolicyConfigXMLParserException, PolicyConfigurationException,
			PolicyFactBuilderException {
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
		...
		PolicyConfigurationUnit policyConfigUnit = new PolicyConfigurationUnit(reqCtx.getTenantId(), reqCtx.getSiteId(),
	...
	@Override
	public boolean deletePolicy(ConfigurationContext configurationContext, String policyName)
			throws PolicyConfigurationException {
		...
		try {
			...
			logger.debug(
					".deletePolicy() deleted from DataGrid psGroupKey=" + psGroupKey + " policyName=" + policyName);
	 ...
	 * @throws PolicyConfigurationException
	 */
	private boolean deletePolicyFromDb(String configName, int nodeId) throws PolicyConfigurationException {
		// Delete from the DB
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
		logger.debug(".deletePolicyFromDb() deleted from db configName=" + configName);
		try {
	...
	/**
	 * based on the PolicyRequestContext and policy name searching Policy ,if
	 * policy Exist in DB check wether it enabled or not , if enabled check
	 * exist in cache or not , if not Exist load the data to cache and return
	 * true else false
	 * 
	 ...
	 * 
	 */
	public boolean checkPolicyExistInDbAndCache(ConfigurationContext configurationContext, String policyName)
			throws PolicyConfigurationException, PolicyRequestException {

		logger.debug("Inside checkPolicyExistInDbAndCache  with configurationContext= " + configurationContext
				+ " policyname=" + policyName);
		PolicyRequestContext requestContext = new PolicyRequestContext(configurationContext.getTenantId(),
				configurationContext.getSiteId(), configurationContext.getFeatureGroup(),
				configurationContext.getFeatureName(), configurationContext.getImplementationName(),
				configurationContext.getVendorName(), configurationContext.getVersion());
		...
		try {
			...
			if (isEnabled) {
				try {
					...
					throw new PolicyConfigurationException("Error in loading policy data into cache with policyName="
							+ policyName + " requestContext=" + requestContext);
				}
			}
		} catch (InvalidNodeTreeException | ConfigPersistenceException e) {
			throw new PolicyRequestException("Error in Searching the policy with policyName=" + policyName
