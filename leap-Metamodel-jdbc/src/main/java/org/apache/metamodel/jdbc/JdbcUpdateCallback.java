...
abstract class JdbcUpdateCallback extends AbstractUpdateCallback implements UpdateCallback {
	...
	public int executePreparedStatement(PreparedStatement preparedStatement, boolean reusedStatement,
		...
		if (collectGeneratedKeys && isGeneratedKeysCollectionEnabled()) {
			try {
				...
				// ignore
				logger.debug("Getting generated keys from JDBC statement is not supported by driver: {}",
						e.getMessage(), e);
	...
	public final void close(boolean success) {
		if (_connection != null) {
			...
			try {
				...
				if (getJdbcDataContext().isDefaultAutoCommit() && !getJdbcDataContext().isInTransaction()) {
					try {
	...
	private void commitOrRollback(boolean success) {
		...
		if (success) {
			try {
				if (!getJdbcDataContext().isInTransaction())
	...
	private final PreparedStatement createPreparedStatement(String sql, boolean returnGeneratedKeys) {
		try {
			...
			if (returnGeneratedKeys) {
				// not all databases support the RETURN_GENERATED_KEYS flag, so
				// retry without
	...
	@Override
	public final RowDeletionBuilder deleteFrom(Table table)
	...
	@Override
	public final RowUpdationBuilder update(Table table)
			throws IllegalArgumentException, IllegalStateException, UnsupportedOperationException {
